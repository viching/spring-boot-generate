...
@Component
public class PreSelectBySelective extends GenerateProvider {

	public PreSelectBySelective() {
		super();
	}
	...
	public void addClassElements(TopLevelClass topLevelClass, DBTableJavaBean dbTable) {
		...
		Method method = new Method(GenerateMethod.METHOD_SELECT_SELECTIVE);
		...
		for (DBColumnJavaBean dbColumn : dbTable.getAllColumns()) {
			method.addBodyLine(String.format("%sSELECT(\"%s\");", builderPrefix, dbColumn.getColumnName()));
		}
